...
app.get("/jokes/:id", (req, res) => {
  ...
  // url parameters come in as req.params (/2) or req.query (?=2), parseInt converts parsed data into integer type
  const foundJoke = jokes.find((joke) => joke.id === id); 
  // array.find(callback) method loops through array checking each entry to see if its id value is equal to the id received from url parameter and stores the callback value in a variable.
  res.json(foundJoke);
});
...
app.patch("/jokes/:id", (req,res) => {
  ...
  const replacementJoke = {
    id: id,
    jokeText: req.body.text || existingJoke.text,
    jokeType: req.body.type || existingJoke.type,
  };
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
